...
module MakeDividers(div) {
  ...
  module MakeDivider(title, tab_offset) {
	...
	for (c = [0:num_columns]) {
	  translate([
	    divider_column + (divider_column + gap_size) * c, divider_bottom, 0
	  ])
	  MakeRoundedCubeAxis(
	...
	MakeRoundedCubeAxis([ tab_width, tab_height + height_overlap, depth ],
      ...
      resize([ text_width, 0, 0 ], auto = [ true, true, false ])
          linear_extrude(depth)
              text(text = title, font = font, size = font_size, valign = "top",
  ...
  module MakeLayer(component, layer = "") {
	...
	if (m_component_compartment_custom && !$preview) {
	  union() {
	    ...
	    step_size = (__component_size(k_y) - acc_sep_width) /
	                (m_component_compartment_custom_count);
	    for (i = [0:round(m_component_compartment_custom_count) - 2]) {
	      translate([
		-(g_wall_thickness / 2 + g_tolerance),
		(i + 1) * step_size + i * m_component_compartment_custom_width,
		m_component_base_height
	      ])
	      cube([
		__component_size(k_x) + g_wall_thickness + 2 * g_tolerance,
		m_component_compartment_custom_width,
		m_box_size[k_z] + __lid_external_size(k_z)
	      ]);
	    }
