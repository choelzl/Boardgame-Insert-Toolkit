...
// thickness of detent. For a looser snap fit, reduce this. For a tighter snap fit, increase it.  ( recommended 0.05
// increments )
...
{
    translate([ 0, 0, height / 2 ]) multmatrix(m = [ [ 1, 0, sin(x), 0 ], [ 0, 1, sin(y), 0 ], [ 0, 0, 1, 0 ] ])
...
module MakeBox(box)
{
    ...
    m_wall_thickness =
        g_b_fit_test ? 0.5
                     : __value(box, "wall_thickness", default = g_wall_thickness); // needs work to change if no lid
    ...
    m_box_multiple_count = __value(box, BOX_MULTIPLE, default = 1);
    m_box_multiple_lid_count = __value(box, BOX_MULTIPLE_LID, default = 1);
    ...
    module MakeLayer(component, layer = "")
    {
        ...
        m_component_compartiment_custom = __value(component, CMP_COMPARTMENT_CUSTOM_B, default = false);
        ...
        module ContainWithinBox()
        {
            ...
            {
                echo("<br><font color='red'>WARNING: Components in RED do not fit in box.</font><br>");
        ...
        module InnerLayer()
        {
            ...
            if (m_is_component_subtractions)
            {
                ...
                union()
                {
                    ...
                    if (m_component_compartiment_custom)
                    {
                        for (i = [0:round(__component_size(k_y))])
                        {
                            if (i % 4 == 0)
                                translate([ -2, i, 0 ])
                                    cube([ __component_size(k_x) + 4, 2, m_box_size[k_z] + __lid_external_size(k_z) ]);
                        }
                    }
                }
            }
            ...
            else if (m_is_final_component_subtractions)
            {
                ...
                if (m_component_has_side_cutouts)
                {
                    ...
                    // Count how many cutouts are made in each direction to substract it from the size of the inner
                    // stencil box
            ...
            module Helper__BuildLid()
            {
                ...
                else // new lid
                {
                    if (m_lid_inset)
                    {
                        ...
                        //     // hollow the center
                        //     MoveToLidInterior()
                        //         cube([  __lid_internal_size( k_x ), __lid_internal_size( k_y ),  __lid_external_size(
                        //         k_z )]);
        ...
        module AddFillets()
        {
            ...
            module _MakeFillet()
            {
                difference()
                {
                    cube_rotated = __component_shape_rotated_90() ? [ __compartment_size(k_x), r, r ]
                                                                  : [ r, __compartment_size(k_y), r ];
                    cube(cube_rotated);

                    cylinder_translated =
                        __component_shape_rotated_90() ? [ 0, r, r ] : [ r, __compartment_size(k_y), r ];
            ...
            mirrorpt = __component_shape_rotated_90() ? [ 0, __compartment_size(k_y) / 2, 0 ]
                                                      : [ __compartment_size(k_x) / 2, 0, 0 ];
