...
function __value(table, key, default = false) =
  __index_of_key(table, key) == [] ? default
...
module
RotateAndMoveBackToOrigin(a, extents)
{
  ...
  translate(pos) rotate(a = a, v = [ 0, 0, 1 ]) children();
}

module
RotateAboutPoint(a, v, pt)
{
  translate(pt) rotate(a, v) translate(-pt) children();
}

module
MirrorAboutPoint(v, pt)
{
  translate(pt) mirror(v) translate(-pt) children();
}

function __element(i) = data[i][1];
function
__num_elements() = len(data);

function __type(lmnt) = __value(lmnt, TYPE, default = BOX);

function
__is_element_isolated_for_print() =
  g_isolated_print_box != "" &&
...
function __label_image(label, r = 0, c = 0) =
  !__is_text(label)
    ? (__is_multiimage(label) ? __value(label, LBL_IMAGE, default = "")[c][r]
...
Shear(x, y, height)
{
  translate([ 0, 0, height / 2 ])
    multmatrix(m = [ [ 1, 0, sin(x), 0 ], [ 0, 1, sin(y), 0 ], [ 0, 0, 1, 0 ] ])
  ...
  module MakeDivider(title, tab_offset)
  {
    ...
    difference()
    {
      ...
      if (num_columns != -1)
        for (c = [0:num_columns]) {
          translate([
            divider_column + (divider_column + gap_size) * c,
            divider_bottom,
            0
          ])
            MakeRoundedCubeAxis(
              [ gap_size, height - divider_bottom - divider_top, depth ],
              4,
              k_z);
        }
    }
    ...
    difference()
    {
      ...
      translate(text_pos)
        resize([ text_width, 0, 0 ], auto = [ true, true, false ])
          linear_extrude(depth) text(text = title,
                                     font = font,
                                     size = font_size,
                                     valign = "top",
                                     halign = "center",
...
MakeBox(box)
{
  ...
  m_lid_tab = [
    max(m_box_size[k_x] / 4, g_detent_spacing * 2),
    m_wall_thickness,
  ...
  module MakeCorners(mod = 0)
  {
    difference()
    {
      ...
      translate([ 0, m_corner_width + mod, 0 ]) cube([
        m_box_size[k_x],
        m_box_size[k_y] - 2 * (m_corner_width + mod),
        m_box_size[k_z] + __lid_external_size(k_z)
      ]); // middle y

      translate([ m_corner_width + mod, 0, 0 ]) cube([
        m_box_size[k_x] - 2 * (m_corner_width + mod),
        m_box_size[k_y],
        m_box_size[k_z] + __lid_external_size(k_z)
      ]); // middle x
  ...
  module MakeLayer(component, layer = "")
  {
    ...
    m_component_cutout_side = __value(
      component, CMP_CUTOUT_SIDES_4B, default = [ false, false, false, false ]);
    ...
    function __component_fillet_radius() = __value(
      component, CMP_FILLET_RADIUS, default = min(__compartment_size(k_z), 10));
    ...
    {
      r =
        !g_b_colorize ? 0.7 : pow(sin(pow(__component_position(k_x), 5)), 0.5);
      g =
        !g_b_colorize ? 0.8 : pow(sin(pow(__component_position(k_y), 5)), 0.3);
    ...
    module PositionInnerLayer()
    {
      ContainWithinBox() RotateAboutPoint(
        __component_rotation(),
        [ 0, 0, 1 ],
        [
          __component_position(k_x) + __component_size(k_x) / 2,
          __component_position(k_y) + __component_size(k_y) / 2,
          0
        ])
        translate([
          ...
          m_wall_thickness
        ]) Shear(__component_shear(k_x),
                 __component_shear(k_y),
    ...
    if (__is_component_enabled()) {
      ...
      if (g_print_mmu_layer == "mmu_label_layer") {
        ...
        if (!m_lid_inset && m_box_has_lid) {
          ...
          if (m_lid_notches)
            translate([ 0, 0, notch_pos_z ]) MakeLidCornerNotches();
        }
    ...
    module MakeLidBase_Inset(
      tolerance = 0, tolerance_detent_pos = 0, omit_detents = false)
    {
      difference()
      {
        // main __element
        cube([
          __lid_external_size(k_x),
        ...
        difference()
        {
          ...
          translate([ 0, 0, 0 ]) MirrorAboutPoint(
            v = [ 0, 0, 1 ],
            pt =
      ...
      difference()
      {
        ...
        translate([
          ...
          cube([
            ...
            __lid_internal_size(k_y) + 2 * tolerance,
            __lid_external_size(k_z)
          ]);
      }
      ...
      difference()
      {
        union()
        {
          ...
          if (m_box_has_lid || m_box_is_stackable) {
            ...
            translate([ 0, 0, m_box_size[k_z] ]) difference()
            {
              cube(
                [ m_box_size[k_x], m_box_size[k_y], __lid_external_size(k_z) ]);
    ...
    {
      {
        difference()
        {
          ...
          translate([ m_wall_thickness, m_wall_thickness, 0 ]) cube([
            m_box_size[k_x] - (2 * m_wall_thickness),
            m_box_size[k_y] - (2 * m_wall_thickness),
            m_box_size[k_z]
          ]);
      ...
      if (m_is_component_subtractions) {
        ...
        InEachCompartment()
        {
          ...
          if (__component_is_fillet()) {
            translate([ 0, 0, m_component_base_height ]) AddFillets();
          }
        }
        ...
        if (m_push_base && m_component_base_height > 0) {
          ...
          InEachCompartment() translate([
            ...
            (__compartment_size(k_y) * (1 - frac)) / 2,
            0
        ...
        if (m_component_compartment_custom && !$preview) {
          ...
          {
            step_size =
              (__component_size(k_y) - m_component_compartment_custom_width) /
              (m_component_compartment_custom_count - 1);
            for (i = [0:round(m_component_compartment_custom_count - 1)]) {
              translate([
                -(g_wall_thickness + g_tolerance),
                i * step_size,
                m_component_base_height
              ])
                cube([
                  __component_size(k_x) + 2 * (g_wall_thickness + g_tolerance),
          ...
          if (m_component_cutout_type == BOTH) {
            ...
            intersection()
            {
              ...
              difference()
              {
                ...
                translate([ insetx, insety, -m_wall_thickness ]) cube(
        ...
        if (!g_b_no_labels_actual) {
          LabelEachCompartment();
                 ...
                 !$preview) {
        translate([ 0, 0, -m_wall_thickness ]) cube([
          __component_size(k_x),
          __component_size(k_y),
          m_box_size[k_z] + __lid_external_size(k_z)
        ]);
    ...
    module Make2DPattern(x = 200, y = 200, R = 1, t = 0.5)
    {
      ...
      for (j = [-1:y_count + 1])
        translate([ (j % 2) * dx / 2 + t, 0, 0 ]) for (i = [-1:x_count + 1])
    ...
    {
      translate([
        m_lid_wall_thickness + tolerance,
        m_lid_wall_thickness + tolerance,
        0
      ]) children();
    }
    ...
    module MakeLidLabel(label, offset = 0, thickness = m_lid_thickness)
    {
      ...
      auto_width = __label_auto_width(
        label, __lid_external_size(k_x), __lid_external_size(k_y));
    ...
    {
      width = __label_auto_width(label,
                                 m_box_size[k_x] - m_lid_notch_height,
      ...
      RotateAboutPoint(__label_rotation(label), [ 0, 1, 0 ], [ 0, 0, 0 ])
        RotateAboutPoint(90, [ 1, 0, 0 ], [ 0, 0, 0 ]) translate(
          ...
          resize([ width, 0, 0 ],
                 auto = true) translate([ 0, 0, -__label_depth(label) ])
    ...
    module MakeLidLabelFrame(label, offset = 0, thickness = m_lid_thickness)
    {
            ...
            offset(r = offset) intersection()
      {
        hull()
        {
          translate([ -200, 0, 0 ]) Make2dLidLabel(label, width, offset);

          translate([ 200, 0, 0 ]) Make2dLidLabel(label, width, offset);
        }
        hull()
        {
          translate([ -0, -200, 0 ]) Make2dLidLabel(label, width, offset);
                ...
                difference()
                {
                  intersection()
                  {
                    ...
                    translate([ x / 2 - x2 / 2, y / 2 - y2 / 2, 0 ])
                      RotateAboutPoint(
                        theta, [ 0, 0, 1 ], [ x2 / 2, y2 / 2, 0 ])
                        MakeStripedGrid(x = x2,
                                        y = y2,
                                        w = m_lid_stripe_width,
                                        dx = m_lid_stripe_space,
                                        dy = 0,
                                        depth_ratio = 0.5,
                                        thickness = thickness);
          ...
          if (m_lid_inset) {
            ...
            difference(tolerance = g_tolerance)
            {
              ...
              MoveToLidInterior() cube([
                __lid_internal_size(k_x),
                __lid_internal_size(k_y),
                __lid_external_size(k_z)
              ]);
            }
    ...
    module ForEachPartition(D, )
    {
      ...
      if (end >= start) {
        ...
        for (a = [start:end]) {
          ...
          translate([ D == k_x ? pos : 0, D == k_y ? pos : 0, 0 ]) children();
        }
      }
    }
    ...
    module ForEachCompartment(D)
    {
...

      {
        for (a = [start:end]) {
          ...
          translate([ D == k_x ? dim1 : 0, D == k_y ? dim1 : 0, 0 ]) children();
        }
      }
    }
    ...
    module LabelEachCompartment()
    {
      ...
      for (x = [0:n_x - 1]) {
        ...
        for (y = [0:n_y - 1]) {
          ...
          translate(
            [ x_pos, y_pos,
              m_component_base_height ]) // to compartment origin
          {
    ...
    module MakeSideCutouts(side, margin = false)
    {
      ...
      function __cutout_z() = m_is_lid
                                ? m_lid_wall_height + m_lid_thickness
                                : m_box_size[k_z] + __lid_external_size(k_z) *
                                                      2 * m_cutout_height_pct;
      ...
      main_size =
        margin
          ? m_component_margin_side[side] + g_wall_thickness + radius
      ...
      translate(pos[side]) if (__round_bottom())
      {
      ...
      else MakeRoundedCubeAxis(size[side], radius, shape, k_z);
    }
    ...
    module MakeCornerCutouts(corner)
    {
      ...
      translate(pos[corner]) MakeRoundedCubeAxis(size, 3, shape[corner], k_z);
    }
    ...
    module MakeVerticalShape(h, x, r)
    {
      ...
      translate(cylinder_translation)
      {
        ...
        rotate(a = angle, v = [ 0, 0, 1 ]) cylinder(h, r, r, center = false);
      }
    }
    ...
    module MakeCompartmentShape()
    {
      ...
      if (__component_is_square()) {
        ...
        MakeVerticalShape(h = __compartment_size(k_z) +
                              m_component_base_height + epsilon,
                          x = x,
                          r = r);
      } else {
        ...
        translate([ 0, 0, m_component_base_height + epsilon ]) union()
        {
          cylinder_translation =
    ...
    module Helper_MakeLabel(label, x = 0, y = 0)
    {
      ...
      width = __label_auto_width(
        label, __compartment_size(k_x), __compartment_size(k_y));
      ...
      RotateAboutPoint(
        label_needs_to_be_180ed ? 180 : 0, [ 0, 0, 1 ], [ 0, 0, 0 ])
        RotateAboutPoint(__label_rotation(label), rotate_vector, [ 0, 0, 0 ])
          RotateAboutPoint(label_on_side ? 90 : 0, [ 1, 0, 0 ], [ 0, 0, 0 ])
            translate(
              [ __label_offset(label)[k_x], __label_offset(label)[k_y], 0 ])
              resize([ width, 0, 0 ], auto = true)
                translate([ 0, 0, -__label_depth(label) ]) linear_extrude(
                  height = __label_depth(label)) if (__is_text(label))
      {
    ...
    module MakeMargin(side)
    {
      if (side == k_left)
        ...
        translate(
          ...
          cube([
            m_component_margin_side[k_right],
            __component_size(k_y),
            __partition_height(k_x) +
            m_component_base_height
          ]);
        ...
        translate(
          ...
          cube([
            __component_size(k_x),
            m_component_margin_side[k_back],
            __partition_height(k_x) +
            m_component_base_height
          ]);
    }
      ...
      {
        {
          ...
          {
            cube(
              [ __notch_length(k_x), __lid_notch_depth(), m_lid_notch_height ]);
            cube(
              [ __lid_notch_depth(), __notch_length(k_y), m_lid_notch_height ]);
          }
        }
      }
    ...
    module MakeLidTabs(mod = 0.0, square = false)
    {
      module MakeLidTab(mod)
      {
        ...
        // square part
        translate([ -x / 2, 0, 0 ]) cube([ x, y, z ], center = false);
        ...
        if (square) {
          ...
          translate([ -x / 2, 0, z ])
          {
            hull()
            {
              ...
              translate([ 0, 0, y ]) rotate(v = [ 0, 1, 0 ], a = 90)
                cylinder(h = x, r = 0.01);
            }
          }
      }
    ...
    module MakeDetents(mod = 0, offset = 0)
    {
      ...
      module MakeOneSet(mod)
      {
        ...
        translate([ 0, offset, 0 ]) hull() for (i = [0:num_detents - 1])
          translate([
            m_wall_thickness / 2 + g_detent_spacing * i +
              g_detent_dist_from_corner,
            m_wall_thickness / 2,
            0
          ]) rotate(90) MakeDetent(mod);

        translate([ offset, 0, 0 ]) hull() for (i = [0:num_detents - 1])
          translate([
            m_wall_thickness / 2,
            m_wall_thickness / 2 + g_detent_spacing * i +
              g_detent_dist_from_corner,
...
module
MakeRoundedCubeAxis(vec3, radius, vecRounded = [ t, t, t, t ], axis = k_z)
{
  radii = [
  ...
  hull()
  {
    ...
    for (idx = [0:3]) {
      ...
      translate(pos[axis][idx]) rotate(rot[axis])
...
module
MakeRoundedCubeAll(vecCube, radius, axis = k_z, vecRounded = [ t, t, t, t ])
{
  ...
  translate(vecCubeCtr) debug();

  RotateAboutPoint(a = 90,
                   v = [ axis == k_x ? 1 : 0, axis == k_y ? 1 : 0, 0 ],
                   pt = vecCubeCtr) hull()
  {
    h = vecCube2[k_z];
    ...
    for (idx = [0:3]) {
      ...
      translate(pos[idx]) cylinder(r = radii[idx], h = h, $fn = fn);
    }
  }
}