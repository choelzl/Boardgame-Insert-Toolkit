...
function __value(table, key, default = false) = __index_of_key(table, key) == []
                                                    ? default
...
function __is_element_isolated_for_print() = g_isolated_print_box != "" &&
...
function __element_position_x(i) = __element(i - 1) == undef ? 0
...
function __label_text(label, r = 0, c = 0) = __is_text(label)
                                                 ...
                                                 : "";
function __label_image(label, r = 0, c = 0) = !__is_text(label) ? (__is_multiimage(label)
...
function __label_rotation(label) = __label_rotation_raw(label) +
                                   (__label_placement_is_left(label)                                        ? 90
                                    ...
                                    : __label_placement_is_front(label) && __label_placement_is_wall(label) ? 0
                                                                                                            : 0);
...
function __label_placement_is_back(label) = __label_placement_raw(label) == BACK ||
                                            __label_placement_raw(label) == BACK_WALL;
function __label_placement_is_front(label) = __label_placement_raw(label) == FRONT ||
                                             __label_placement_raw(label) == FRONT_WALL;
function __label_placement_is_left(label) = __label_placement_raw(label) == LEFT ||
                                            __label_placement_raw(label) == LEFT_WALL;
function __label_placement_is_right(label) = __label_placement_raw(label) == RIGHT ||
                                             __label_placement_raw(label) == RIGHT_WALL;
function __label_placement_is_bottom(label) = __label_placement_raw(label) == BOTTOM;
function __label_placement_is_wall(label) = __label_placement_raw(label) == BACK_WALL ||
                                            __label_placement_raw(label) == FRONT_WALL ||
                                            __label_placement_raw(label) == LEFT_WALL ||
                                            __label_placement_raw(label) == RIGHT_WALL;
...
function __label_auto_width(label, x,
                            y) = __label_size_is_auto(label)
...
{
    translate([ 0, 0, height / 2 ]) multmatrix(m = [ [ 1, 0, tan(x), 0 ], [ 0, 1, sin(y), 0 ], [ 0, 0, 1, 0 ] ])
...
module MakeAll()
{
    ...
    else
    {
        for (i = [0:__num_elements() - 1])
        {
            ...
            element_rotation =
                (g_b_vis_actual && __box_vis_rotation(element) != undef) ? __box_vis_rotation(element) : 0;

            translate(element_position) RotateAndMoveBackToOrigin(element_rotation, __element_dimensions(i))
            {
...
module MakeDividers(div)
{
    ...
    for (idx = [0:len(tab_text) - 1])
    {
        ...
        translate([ 0, y_offset, 0 ]) MakeDivider(title = tab_text[idx], tab_offset = tab_offset);
    }
    ...
    module MakeDivider(title, tab_offset)
    {
        ...
        difference()
        {
            ...
            translate(text_pos) resize([ text_width, 0, 0 ], auto = [ true, true, false ]) linear_extrude(depth)
                text(text = title, font = font, size = font_size, valign = "top", halign = "center",
                     spacing = font_spacing, $fn = fn);
        }
    }
}
...
module MakeBox(box)
{
    ...
    m_wall_thickness = g_b_fit_test ? 0.5
                                    : __value(box, "wall_thickness",
    ...
    m_box_multiple_lid_count = $preview ? 1 : __value(box, BOX_MULTIPLE_LID, default = 1);
    ...
    function __lid_external_size(D) = D == k_z ? m_lid_thickness + m_lid_wall_height : m_box_size[D];

    m_has_solid_lid = m_lid_solid || g_b_vis_actual;
    m_lid_has_labels = !!__value(m_lid, LABEL, default = false);

    function __lid_internal_size(D) = D == k_z ? m_lid_wall_height : __lid_external_size(D) - 2 * m_lid_wall_thickness;
    ...
    m_lid_is_inverted_fill = __value(m_lid, LID_LABELS_INVERT_FILL_B, default = false);
    ...
    module MakeCorners(mod = 0)
    {
        difference()
        {
            cube([ m_box_size[k_x], m_box_size[k_y],
                   m_box_size[k_z] + __lid_external_size(k_z) ]); // outer box
            ...
            translate([ m_wall_thickness, m_wall_thickness, 0 ]) // innerbox
                cube([
                    m_box_size[k_x] - 2 * m_wall_thickness, m_box_size[k_y] - 2 * m_wall_thickness,
                    m_box_size[k_z] + __lid_external_size(k_z)
                ]);
        }
    }
    ...
    else
    {
        if (g_b_print_lid && m_box_has_lid && m_box_multiple_lid_count > 0)
        {
            ...
            {
                translate([ 0, (m_box_size[k_y] + 2 * g_wall_thickness) * (i + m_box_multiple_count - 1), 0 ])
        ...
        if (g_b_print_box && m_box_multiple_count > 0)
        {
            ...
            {
                translate([ 0, (m_box_size[k_y] + 2 * g_wall_thickness) * i, 0 ]) difference()
                {
                            ...
                            difference()
                            {
                                ...
                                union()
                                {
                                    for (i = [0:m_num_components - 1])
                                    {
                                        ...
                                        {
                                            component = box[i][k_value];
                                            MakeLayer(component, layer = "component_background_cutout");
                                        }
                                    }
                                }
                            }
    ...
    module MakeLayer(component, layer = "")
    {
        ...
        function __compartment_size(D) =
            __value(component, CMP_COMPARTMENT_SIZE_XYZ, default = [ 10.0, 10.0, 10.0 ])[D];
        ...
        function __component_margins_sum(D) =
            D == k_x   ? m_component_margin_side[k_left] + m_component_margin_side[k_right]
        ...
        m_component_has_side_cutouts = m_component_cutout_side[k_front] || m_component_cutout_side[k_back] ||
                                       m_component_cutout_side[k_left] || m_component_cutout_side[k_right];

        m_component_background = __value(component, CMP_LID_BACKGROUND_B, default = false);
        m_component_compartment_custom = __value(component, CMP_COMPARTMENT_CUSTOM_B, default = false);
        m_component_compartment_custom_count = __value(component, CMP_COMPARTMENT_CUSTOM_COUNT, default = 4);
        ...
        m_component_has_exactly_one_cutout =
            (m_component_cutout_side[k_front] ? 1 : 0) + (m_component_cutout_side[k_back] ? 1 : 0) +
                (m_component_cutout_side[k_left] ? 1 : 0) + (m_component_cutout_side[k_right] ? 1 : 0) ==
            1;
        ...
        function __component_padding_height_adjust(D) =
            __value(component, CMP_PADDING_HEIGHT_ADJUST_XY, default = [ 0.0, 0.0 ])[D];
        ...
        function __component_fillet_radius() =
            __value(component, CMP_FILLET_RADIUS, default = min(__compartment_size(k_z), 10));
        m_component_fillet_sides = __value(component, CMP_FILLET_SIDES_4B, default = [ f, t, f, t ]);
        ...
        function __compartment_smallest_dimension() =
            (__compartment_size(k_x) < __compartment_size(k_y)) ? __compartment_size(k_x) : __compartment_size(k_y);
        ...
        function __component_size(D) =
            (D == k_z) ? __compartment_size(k_z)
                       : (__compartment_size(D) * __compartments_num(D)) +
                             (__partitions_num(D) * __component_padding(D) + __component_margins_sum(D));
        ...
        {
            ContainWithinBox() RotateAboutPoint(__component_rotation(), [ 0, 0, 1 ],
                                                [
                ...
                translate([ __component_position(k_x), __component_position(k_y), m_wall_thickness ])
                    Shear(__component_shear(k_x), __component_shear(k_y), __component_size(k_z)) children();
        }
        ...
        if (__is_component_enabled())
        {
            ...
            else if (m_is_lid_subtractions)
            {
                ...
                if (m_lid_fit_under && m_box_has_lid)
                {
                    ...
                    {
                        translate([ 0, 0, -m_lid_thickness ]) MakeLidBase_Cap(omit_detents = false);
                    }
                }
            }
        ...
        module MakeLidBase_Inset(tolerance = 0, tolerance_detent_pos = 0, omit_detents = false)
        {
            difference()
            {
                ...
                translate([ 0, 0, __lid_external_size(k_z) / 2 ])
                {
                    ...
                    MirrorAboutPoint(
                        v = [ 0, 1, 0 ],
                        pt = [ __lid_external_size(k_x) / 2, __lid_external_size(k_y) / 2, m_lid_wall_height / 2 ])
                        MirrorAboutPoint(
                            v = [ 1, 0, 0 ],
                            pt = [ __lid_external_size(k_x) / 2, __lid_external_size(k_y) / 2, m_lid_wall_height / 2 ])
                    {
                        cube([
                            __lid_external_size(k_x), __lid_notch_depth() + tolerance, __lid_external_size(k_z) / 1
                        ]);
                        cube([
                            __lid_notch_depth() + tolerance, __lid_external_size(k_y), __lid_external_size(k_z) / 1
                        ]);
                    }
                }
                ...
                difference()
                {
                    ...
                    translate([ 0, 0, 0 ]) MirrorAboutPoint(
                        v = [ 0, 0, 1 ], pt = [ __lid_external_size(k_x) / 2, __lid_external_size(k_y) / 2, 0 ]) hull()
                    {
        ...
        module MakeLidBase_Cap(tolerance = 0, tolerance_detent_pos = 0, omit_detents = false)
        {
            difference()
            {
                ...
                // #TODO: modulize this!
                translate([ 0, 0, __lid_external_size(k_z) - __lid_notch_depth() ]) hull()
                {
                ...
                // big hollow
                translate([ __lid_notch_depth() - tolerance, __lid_notch_depth() - tolerance, 0 ]) cube([
                    __lid_internal_size(k_x) + 2 * tolerance, __lid_internal_size(k_y) + 2 * tolerance,
        ...
        module MakeLidEdge(extra_height = 0, extra_depth = 0, offset = 0)
        {
            ...
            {
                cube([
                    m_box_size[k_x] - (2 * offset), __lid_notch_depth() + extra_depth, m_lid_wall_height + extra_height
                ]);

                cube([
                    __lid_notch_depth() + extra_depth, m_box_size[k_y] - (2 * offset), m_lid_wall_height + extra_height
                ]);
            }
        }
        ...
        module InnerLayer()
        {
            ...
            if (m_is_component_subtractions)
            {
                ...
                union()
                {
                    cube([ __component_size(k_x), __component_size(k_y), m_box_size[k_z] + __lid_external_size(k_z) ]);
                }
            }
            else if (m_is_component_additions)
            {
                ...
                InEachCompartment()
                {
                    if (!__component_is_square() && !__component_is_fillet())
                    {
                        ...
                        {
                            cube([
                                __compartment_size(k_x), __compartment_size(k_y), __smallest_partition_height() +
                ...
                if (m_push_base && m_component_base_height > 0)
                {
                    ...
                    InEachCompartment() translate(
                        [ (__compartment_size(k_x) * (1 - frac)) / 2, (__compartment_size(k_y) * (1 - frac)) / 2, 0 ])
            ...
            else if (m_is_final_component_subtractions)
            {
                ...
                if (m_component_compartment_custom && !$preview)
                {
                    union()
                    {
                        step_size = (__component_size(k_y) - m_component_compartment_custom_width) /
                                    (m_component_compartment_custom_count - 1);
                        for (i = [0:round(m_component_compartment_custom_count - 1)])
                        {
                            translate([ -(g_wall_thickness + g_tolerance), i * step_size, m_component_base_height ])
                                cube([
                                    __component_size(k_x) + 2 * (g_wall_thickness + g_tolerance),
                                    m_component_compartment_custom_width, m_box_size[k_z] + __lid_external_size(k_z)
                                ]);
                        }
                    }
                }
                ...
                {
                    // finger cutouts
                    insetx = m_component_cutout_side[k_left] ? m_cutout_size_frac_aligned * __compartment_size(k_x) +
                                                                   m_component_margin_side[k_left]
                                                             : 0;
                    insety = m_component_cutout_side[k_front] ? m_cutout_size_frac_aligned * __compartment_size(k_y) +
                    ...
                    sizex = __component_size(k_x) - x_cutouts * m_cutout_size_frac_aligned * __compartment_size(k_x) -
                            __component_margins_sum(k_x);
                    sizey = __component_size(k_y) - y_cutouts * m_cutout_size_frac_aligned * __compartment_size(k_y) -
                    ...
                    else if (m_component_cutout_type == INTERIOR)
                    {
                        intersection()
                        {
                            ...
                            translate([ insetx, insety, -m_wall_thickness ]) cube([ sizex, sizey, m_box_size[k_z] ]);
                    ...
                    else if (m_component_cutout_type == EXTERIOR)
                    {
                        intersection()
                        {
                            ...
                            difference()
                            {
                                ...
                                translate([ -__component_position(k_x), -__component_position(k_y), -m_wall_thickness ])
                                    cube([
                                        m_box_size[k_x], m_box_size[k_y], m_box_size[k_z] + __lid_external_size(k_z)
                                    ]);
                ...
                InEachCompartment()
                {
                    ...
                    if (m_actually_cutout_the_bottom)
                        translate([
                            (__compartment_size(k_x) * (1 - frac)) / 2, (__compartment_size(k_y) * (1 - frac)) / 2, -
                            m_wall_thickness
                        ]) scale(v = [ frac, frac, 1 ]) // bring in the sides
                            MakeCompartmentShape();
                }
            ...
            else if (m_is_component_background && m_component_background && !$preview)
            {
                translate([ 0, 0, -m_wall_thickness ])
                    cube([ __component_size(k_x), __component_size(k_y), m_box_size[k_z] + __lid_external_size(k_z) ]);
            }
            else if (m_is_component_background_cutout && m_actually_cutout_the_bottom)
            {
                frac = 0.9;
                translate([
                    (__compartment_size(k_x) * (1 - frac)) / 2, (__compartment_size(k_y) * (1 - frac)) / 2, -
        ...
        module Tri(R = 1, t = 0.2)
        {
            ...
            polygon([[R * cos(0 * a), R * sin(0 * a)], [R * cos(1 * a), R * sin(1 * a)],
                     [R * cos(2 * a), R * sin(2 * a)],

                     [(R - t) * cos(0 * a), (R - t) * sin(0 * a)], [(R - t) * cos(1 * a), (R - t) * sin(1 * a)],
        ...
        module Quad(R = 1, t = 0.2)
        {
            ...
            polygon([[R * cos(0 * a), R * sin(0 * a)], [R * cos(1 * a), R * sin(1 * a)],
                     ...
                     [(R - t) * cos(0 * a), (R - t) * sin(0 * a)], [(R - t) * cos(1 * a), (R - t) * sin(1 * a)],
                     [(R - t) * cos(2 * a), (R - t) * sin(2 * a)], [(R - t) * cos(3 * a), (R - t) * sin(3 * a)],
            ],
        ...
        module Hex(R = 1, t = 0.2)
        {
            ...
            polygon([[R * cos(0 * a), R * sin(0 * a)], [R * cos(1 * a), R * sin(1 * a)],
                     ...
                     [(R - t) * cos(0 * a), (R - t) * sin(0 * a)], [(R - t) * cos(1 * a), (R - t) * sin(1 * a)],
                     [(R - t) * cos(2 * a), (R - t) * sin(2 * a)], [(R - t) * cos(3 * a), (R - t) * sin(3 * a)],
                     [(R - t) * cos(4 * a), (R - t) * sin(4 * a)], [(R - t) * cos(5 * a), (R - t) * sin(5 * a)]],
        ...
        module Oct(R = 1, t = 0.2)
        {
            ...
            polygon([[R * cos(0 * a), R * sin(0 * a)], [R * cos(1 * a), R * sin(1 * a)],
                     ...
                     [(R - t) * cos(2 * a), (R - t) * sin(2 * a)], [(R - t) * cos(3 * a), (R - t) * sin(3 * a)],
                     [(R - t) * cos(4 * a), (R - t) * sin(4 * a)], [(R - t) * cos(5 * a), (R - t) * sin(5 * a)],
                     [(R - t) * cos(6 * a), (R - t) * sin(6 * a)], [(R - t) * cos(7 * a), (R - t) * sin(7 * a)],
            ],
        ...
        function AddOrderIndex(b, e, i = 0) = i == e ? [] : concat(i, AddOrderIndex(b, e, i + 1));
        ...
        module Make2DPattern(x = 200, y = 200, R = 1, t = 0.5)
        {
            ...
            for (j = [-1:y_count + 1])
                translate([ (j % 2) * dx / 2 + t, 0, 0 ]) for (i = [-1:x_count + 1]) translate([ i * dx, j * dy, 0 ])
        ...
        module MakeStripedGrid(x = 200, y = 200, w = 1, dx = 0, dy = 0, depth_ratio = 0.5, thickness = m_lid_thickness)
        {
            ...
            if (dx > 0)
            {
                for (j = [0:x_count])
                {
                    translate([ j * (w + dx), 0, thickness - _thickness ]) cube([ w, y, _thickness ]);
                }
            }
            ...
            if (dy > 0)
            {
                for (j = [0:y_count])
                {
                    translate([ 0, j * (w + dy), thickness - _thickness ]) cube([ x, w, _thickness ]);
                }
            }
        }
        ...
        {
            translate([ m_lid_wall_thickness + tolerance, m_lid_wall_thickness + tolerance, 0 ]) children();
        }
        ...
        {
            resize([ width, 0, 0 ], auto = true) offset(r = offset) if (__is_text(label))
            {
                text(text = str(__label_text(label)), font = __label_font(label), size = __label_size(label),
                     spacing = __label_spacing(label), valign = CENTER, halign = CENTER, $fn = fn);
            }
            else
            {
        ...
        {
            width =
                __label_auto_width(label, m_box_size[k_x] - m_lid_notch_height, m_box_size[k_y] - m_lid_notch_height);
            ...
            RotateAboutPoint(__label_rotation(label), [ 0, 1, 0 ], [ 0, 0, 0 ])
                RotateAboutPoint(90, [ 1, 0, 0 ], [ 0, 0, 0 ])
                    translate([ __label_offset(label)[k_x], __label_offset(label)[k_y], 0 ])
                        resize([ width, 0, 0 ], auto = true) translate([ 0, 0, -__label_depth(label) ])
                            linear_extrude(height = __label_depth(label)) if (__is_text(label))
            {
                text(text = str(__label_text(label, x)), font = __label_font(label), size = __label_size(label),
                     spacing = __label_spacing(label), valign = CENTER, halign = CENTER, $fn = fn);
            }
            else
            {
        ...
        module MakeBoxLabel(label, x = 0, y = 0)
        {
            ...
            {
                translate([ m_box_size[k_x] / 2, m_box_size[k_y], z_pos_vertical ]) rotate(180, [ 0, 0, 1 ]) children();
            }
            else if (__label_placement_is_left(label))
            {
                translate([ 0, m_box_size[k_y] / 2, z_pos_vertical ]) rotate(-90, [ 0, 0, 1 ]) rotate(-90, [ 0, 1, 0 ])
                    children();
            }
            else if (__label_placement_is_right(label))
            {
                translate([ m_box_size[k_x], m_box_size[k_y] / 2, z_pos_vertical ]) rotate(90, [ 0, 0, 1 ])
                    rotate(90, [ 0, 1, 0 ]) children();
            }
            else if (__label_placement_is_bottom(label))
            {
                translate([ m_box_size[k_x] / 2, m_box_size[k_y] / 2, 0 ]) rotate(90, [ 1, 0, 0 ]) children();
            }
        }
        ...
        module MakeLidLabelFrame(label, offset = 0, thickness = m_lid_thickness)
        {
            ...
            linear_extrude(thickness) translate([ xpos, ypos, 0 ])
                MirrorAboutPoint([ 1, 0, 0 ], [ 0, 0, thickness / 2 ])
                    RotateAboutPoint(__label_rotation(label), [ 0, 0, 1 ], [ 0, 0, 0 ]) offset(r = offset)
                        intersection()
            {
                hull()
                {
        ...
        module MakeLid()
        {
                    ...
                    if (!m_has_solid_lid)
                    {
                        ...
                        if (!m_lid_is_inverted)
                        {
                            ...
                            if (m_lid_label_bg_thickness > 0)
                                ...
                                {
                                    MakeAllLidLabelFrames(offset = m_lid_label_bg_thickness, thickness = thickness);
                                    MakeAllLidLabelFrames(offset =
                                                              m_lid_label_bg_thickness - m_lid_label_border_thickness,
                                                          thickness = thickness);
                                }
                                        ...
                                        translate([ x / 2 - x2 / 2, y / 2 - y2 / 2, 0 ])
                                            RotateAboutPoint(theta, [ 0, 0, 1 ], [ x2 / 2, y2 / 2, 0 ])
                                                MakeStripedGrid(x = x2, y = y2, w = m_lid_stripe_width,
                                                                dx = m_lid_stripe_space, dy = 0, depth_ratio = 0.5,
                                                                thickness = thickness);
            ...
            translate(g_b_vis_actual ? lid_vis_position : lid_print_position)
                RotateAboutPoint(g_b_vis_actual ? 180 : 0, [ 0, 1, 0 ],
                                 [ __lid_external_size(k_x) / 2, __lid_external_size(k_y) / 2, 0 ]) difference()
            {
                Helper__BuildLid();
        ...
        module InEachCompartment()
        {
            ...
            for (x = [0:n_x - 1])
            {
                ...
                {
                    y_pos =
                        m_component_margin_side[k_front] + ((__compartment_size(k_y)) + __component_padding(k_y)) * y;
        ...
        module MakeSideCutouts(side, margin = false)
        {
            ...
            pos_standard = [
                // front
                [ __size(k_x) / 2 - perp_size / 2, -__padding(k_y) / 2, __finger_cutouts_bottom() ],
                // back
                [
                    __size(k_x) / 2 - perp_size / 2, __size(main_d) - main_size + __padding(k_y) / 2,
                    __finger_cutouts_bottom()
                ],
                // left
                [ -__padding(k_x) / 2, __size(k_y) / 2 - perp_size / 2, __finger_cutouts_bottom() ],
                // right
                [
                    __size(main_d) - main_size + __padding(k_x) / 2, __size(k_y) / 2 - perp_size / 2,
                    __finger_cutouts_bottom()
                ],
            ];
            ...
            pos_margin = [
                // front
                [
                    __size(k_x) / 2 - perp_size / 2, -__padding(main_d) / 2 - g_wall_thickness,
                    __finger_cutouts_bottom()
                ],
                // back
                [
                ...
                // left
                [
                    -__padding(main_d) / 2 - g_wall_thickness, __size(k_y) / 2 - perp_size / 2,
                    __finger_cutouts_bottom()
                ],
                // right
                [
        ...
        {
            function __cutout_z() =
                (m_is_lid ? m_lid_wall_height + m_lid_thickness : m_box_size[k_z] + __lid_external_size(k_z) * 2);
            function __padding(D) = m_is_lid ? 0 : __component_padding(D);
            function __size(D) = m_is_lid ? __lid_internal_size(D) : __compartment_size(D);
            function __finger_cutouts_bottom() =
                m_is_lid ? __lid_external_size(k_z) - __cutout_z() : -__lid_external_size(k_z);
            ...
            pos = [
                ...
                // k_back_right
                [ __size(k_x) * inverse_inset, __size(k_y) * inverse_inset, __finger_cutouts_bottom() ],
                // k_back_left
                [
                    __size(k_x) * inverse_inset, -__padding(k_y) / 2, __finger_cutouts_bottom()
            ...
            shape = [
                ...
                [
                    ...
                    !m_component_cutout_corner[k_front_right], !m_component_cutout_corner[k_back_left],
                    t
                ],
            ...
            size = [
                ...
                __padding(k_y) / 2 + __size(k_y) * inset_into_compartment_fraction, __cutout_z()
            ];
        ...
        module AddFillets()
        {
            ...
            module _MakeFillet(side = true)
            {
                difference()
                {
                    cube_rotated = side ? [ __compartment_size(k_x), r, r ] : [ r, __compartment_size(k_y), r ];
                    cube(cube_rotated);

                    cylinder_translated = side ? [ 0, r, r ] : [ r, __compartment_size(k_y), r ];
        ...
        module MakeCompartmentShape()
        {
            ...
            {
                cube([
                    __compartment_size(k_x), __compartment_size(k_y), __compartment_size(k_z) + m_component_base_height
                ]);
            }
            ...
            else
            {
                ...
                translate([ 0, 0, m_component_base_height + epsilon ]) union()
                {
                    ...
                    translate(cylinder_translation)
                    {
                        RotateAboutPoint(a = __component_shape_rotated_90() ? 90 : 0, v = [ 0, 0, 1 ], pt = [ 0, 0, 0 ])
                        {
                            {
                                // lay the hex down
                                rotate(a = 90, v = [ 1, 0, 0 ])
                    ...
                    translate([ 0, 0, r ]) cube([ __compartment_size(k_x), __compartment_size(k_y), m_box_size[k_z] ]);
                }
            }
        }
        ...
        module Helper_MakeLabel(label, x = 0, y = 0)
        {
            ...
            label_y_reverse = (__is_text(label) && __is_multitext(label))     ? len(__value(label, LBL_TEXT)) - y - 1
                              : (!__is_text(label) && __is_multiimage(label)) ? len(__value(label, LBL_IMAGE)) - y - 1
                                                                              : 0;
                    ...
                    RotateAboutPoint(label_on_side ? 90 : 0, [ 1, 0, 0 ], [ 0, 0, 0 ])
                        translate([ __label_offset(label)[k_x], __label_offset(label)[k_y], 0 ])
                            resize([ width, 0, 0 ], auto = true) translate([ 0, 0, -__label_depth(label) ])
                                linear_extrude(height = __label_depth(label)) if (__is_text(label))
            {
                text(text = str(__label_text(label, x, label_y_reverse)), font = __label_font(label),
                     size = __label_size(label), spacing = __label_spacing(label), valign = CENTER, halign = CENTER,
                     $fn = fn);
            }
            else
            {
        ...
        module MakeLabel(label, x = 0, y = 0)
        {
            ...
            {
                translate([ __compartment_size(k_x) / 2, __compartment_size(k_y) / 2, z_pos ]) children();
            }
            else if (__label_placement_is_front(label))
            {
                if (__label_placement_is_wall(label))
                    translate([ __compartment_size(k_x) / 2, 0, z_pos_vertical ]) children();
                else
                    translate(
                        [ __compartment_size(k_x) / 2, -__component_padding(k_y) / 4, __partition_height(k_y) + z_pos ])
                        children();
            }
            else if (__label_placement_is_back(label))
            {
                if (__label_placement_is_wall(label))
                    translate([ __compartment_size(k_x) / 2, __compartment_size(k_y), z_pos_vertical ]) children();
                else
                    translate([
                        __compartment_size(k_x) / 2, __compartment_size(k_y) + __component_padding(k_y) / 4,
            ...
            {
                if (__label_placement_is_wall(label))
                    translate([ 0, __compartment_size(k_y) / 2, z_pos_vertical ]) rotate(90, [ 0, 0, 1 ])
                        rotate(-90, [ 0, 1, 0 ]) children();
                else
                    translate(
                        [ -__component_padding(k_x) / 4, __compartment_size(k_y) / 2, __partition_height(k_y) + z_pos ])
                        children();
            }
            else if (__label_placement_is_right(label))
            {
                if (__label_placement_is_wall(label))
                    ...
                    translate([
                        __compartment_size(k_x) + __component_padding(k_x) / 4, __compartment_size(k_y) / 2,
                        __partition_height(k_y) +
                        z_pos
                    ]) children();
            }
        ...
        module MakeMargin(side)
        {
            if (side == k_left)
                cube([
                    m_component_margin_side[k_left], __component_size(k_y), __partition_height(k_x) +
                    m_component_base_height
                ]);
            else if (side == k_right)
                translate([ __component_size(k_x) - m_component_margin_side[k_right], 0, 0 ]) cube([
                    m_component_margin_side[k_right], __component_size(k_y), __partition_height(k_x) +
                    m_component_base_height
                ]);
            else if (side == k_front)
                cube([
                    __component_size(k_x), m_component_margin_side[k_front], __partition_height(k_x) +
                    m_component_base_height
                ]);
            else if (side == k_back)
                translate([ 0, __component_size(k_y) - m_component_margin_side[k_back], 0 ]) cube([
                    __component_size(k_x), m_component_margin_side[k_back], __partition_height(k_x) +
                    m_component_base_height
                ]);
        }
        ...
        module MakePartition(axis)
        {
            ...
            {
                cube([
                    __component_padding(k_x), __component_size(k_y), __partition_height(k_x) + m_component_base_height
                ]);
            }
            else if (axis == k_y)
            {
                cube([
                    __component_size(k_x), __component_padding(k_y), __partition_height(k_y) + m_component_base_height
                ]);
            }
        }
        ...
        module MakeLidTabs(mod = 0.0, square = false)
        {
            module MakeLidTab(mod)
            {
                ...
                if (square)
                {
                    translate([ -x / 2, 0, z ]) cube([ x, y, y + m_lid_wall_height ], center = false);
                }
                else
                    // prism part
                    translate([ -x / 2, 0, z ])
                    {
                        hull()
                        {
                            ...
                            translate([ 0, 0, y ]) rotate(v = [ 0, 1, 0 ], a = 90) cylinder(h = x, r = 0.01);
                        }
                    }
            }
            ...
            translate([ (m_box_size[k_x]) / 2, 0, 0 ])
            {
                ...
                if (m_lid_tab_sides[k_back])
                    MirrorAboutPoint([ 0, 1, 0 ], [ m_box_size[k_x] / 2, m_box_size[k_y] / 2, 0 ]) MakeLidTab(mod);
            }
            ...
            translate([ 0, (m_box_size[k_y]) / 2, 0 ])
            {
                ...
                if (m_lid_tab_sides[k_right])
                    MirrorAboutPoint([ 1, 0, 0 ], [ m_box_size[k_x] / 2, m_box_size[k_y] / 2, 0 ]) rotate(-90)
                        MakeLidTab(mod);
            }
        }
        ...
        module MakeDetents(mod = 0, offset = 0)
        {
            ...
            {
                resize([ g_detent_thickness * 2 + mod, 1, 1.0 ]) sphere(r = 1, $fn = 12);
            }
            ...
            module MakeOneSet(mod)
            {
                ...
                translate([ 0, offset, 0 ]) hull() for (i = [0:num_detents - 1]) translate([
                    m_wall_thickness / 2 + g_detent_spacing * i + g_detent_dist_from_corner, m_wall_thickness / 2, 0
                ]) rotate(90) MakeDetent(mod);

                translate([ offset, 0, 0 ]) hull() for (i = [0:num_detents - 1]) translate([
                    m_wall_thickness / 2, m_wall_thickness / 2 + g_detent_spacing * i + g_detent_dist_from_corner, 0
                ]) MakeDetent(mod);
            }
...
module MakeRoundedCubeAxis(vec3, radius, vecRounded = [ t, t, t, t ], axis = k_z)
{
    ...
    pos = [
        [[radii [0], 0, radii [0]], [vec3 [k_x] - radii [1], 0, radii [1]], [radii [0], 0, vec3 [k_z] - radii [0]],
        ...
        [[radii [0], radii [0], 0], [vec3 [k_x] - radii [1], radii [1], 0], [radii [2], vec3 [k_y] - radii [2], 0],
    ...
    hull()
    {
        ...
        for (idx = [0:3])
        {
            ...
            translate(pos[axis][idx]) rotate(rot[axis]) cylinder(r = radii[idx], h = h, $fn = fn);
        }
    }
}
...
module MakeRoundedCubeAll(vecCube, radius, axis = k_z, vecRounded = [ t, t, t, t ])
{
    ...
    pos = [[radii [0], radii [0], 0], [vecCube2 [k_x] - radii [1], radii [1], 0],
           [radii [2], vecCube2 [k_y] - radii [2], 0], [vecCube2 [k_x] - radii [3], vecCube2 [k_y] - radii [3], 0]];

    vecCubeCtr = vecCube / 2;

    translate(vecCubeCtr) debug();

    RotateAboutPoint(a = 90, v = [ axis == k_x ? 1 : 0, axis == k_y ? 1 : 0, 0 ], pt = vecCubeCtr) hull()
    {
        h = vecCube2[k_z];
